...
@Component({
  selector: 'app-plotly-rainfall-chart',
  standalone: true,
  imports: [CommonModule, PlotlyChartCardComponent],
  templateUrl: './plotly-rainfall-chart.component.html',
  styleUrls: ['./plotly-rainfall-chart.component.scss']
})
export class PlotlyRainfallChartComponent implements OnChanges {
  ...
  private loadRainfallData(): void {
    this.rainfallService.getHourlyRainfallData(this.startDate, this.endDate, this.groupBy).subscribe({
      next: (response: any) => {
        this.processRainfallData(response);
      },
      error: (error) => {
  ...
  private processRainfallData(response: any): void {
    const data = response.data;
    const unit = response.unit || 'mm';
    ...
    this.chartLayout.title.text = `Rainfall (${groupLabel}) Analysis (${this.startDate} to ${this.endDate})`;
  ...
  private showErrorChart(): void {
    ...
    this.chartLayout.title.text = 'Rainfall Data - Error Loading';
    this.chartLayout.annotations = [{
      text: 'Unable to load rainfall data. Please check your connection and try again.',
      showarrow: false,
      xref: 'paper',
      yref: 'paper',
      x: 0.5,
  ...
  downloadCSV(): void {
    ...
    const headers = ['Period', 'Average Rainfall (mm)'];
    const rows = [headers];
    ...
    link.setAttribute('download', `rainfall_data_${this.startDate}_${this.endDate}.csv`);
  ...
  downloadPNG(): void {
    if (this.chartContainer && this.chartContainer.nativeElement) {
      Plotly.downloadImage(this.chartContainer.nativeElement, {
        format: 'png',
                filename: `rainfall_chart_${this.startDate}_${this.endDate}`,
        height: 500,
        width: 800,
        scale: 2
      });
