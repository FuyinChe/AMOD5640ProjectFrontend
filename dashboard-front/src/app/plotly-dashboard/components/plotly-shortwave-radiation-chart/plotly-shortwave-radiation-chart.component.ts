...
import { ShortwaveRadiationService } from '../../../services/shortwave-radiation.service';
import { PlotlyChartCardComponent } from '../plotly-chart-card/plotly-chart-card.component';

declare var Plotly: any;

@Component({
  selector: 'app-plotly-shortwave-radiation-chart',
  standalone: true,
  imports: [CommonModule, PlotlyChartCardComponent],
  templateUrl: './plotly-shortwave-radiation-chart.component.html',
  styleUrls: ['./plotly-shortwave-radiation-chart.component.scss']
})
export class PlotlyShortwaveRadiationChartComponent implements OnChanges {
  ...
  private initializeChartConfig(): void {
    ...
    this.chartLayout = {
      title: {
        text: 'Shortwave Radiation Over Time',
        font: {
  ...
  private loadShortwaveRadiationData(): void {
    this.shortwaveRadiationService.getShortwaveRadiationData(this.startDate, this.endDate, this.groupBy).subscribe({
      next: (response: any) => {
        this.processShortwaveRadiationData(response);
      },
      error: (error) => {
        console.error('Error loading shortwave radiation data:', error);
        this.showErrorChart();
      }
    });
  }

  private processShortwaveRadiationData(response: any): void {
    const data = response.data;
    const unit = response.unit || 'W/m²';
    ...
    this.chartData = [{
      ...
      type: 'scatter',
      mode: 'lines+markers',
      name: `Average Shortwave Radiation (${unit})`,
      line: {
        color: '#f39c12',
        width: 3
      },
  ...
  private showErrorChart(): void {
    ...
    this.chartLayout.title.text = 'Shortwave Radiation Data - Error Loading';
    this.chartLayout.annotations = [{
      text: 'Unable to load shortwave radiation data. Please check your connection and try again.',
      showarrow: false,
      xref: 'paper',
      yref: 'paper',
      x: 0.5,
  ...
  downloadCSV(): void {
    ...
    const headers = ['Period', 'Average Shortwave Radiation (W/m²)'];
    const rows = [headers];
    ...
    link.setAttribute('download', `shortwave_radiation_data_${this.startDate}_${this.endDate}.csv`);
